 ...
 * @Last Modified time: 2018-04-28 19:05:23
 */
...
exports.send = (currentComment, parentComment) => {
  if (
    parentComment.get('mail') === config.TO_EMAIL ||
    parentComment.get('mail') === config.SMTP_USER ||
    parentComment.get('mail') === config.SENDER_EMAIL
  ) {
  ...
  let emailContent =
    ...
    ')在<a style="text-decoration:none;color: #12ADDB;" href="' +
    config.SITE_URL +
    ...
    '</strong>&nbsp;回复说：</p><div style="background-color: #f5f5f5;padding: 10px 15px;margin:18px 0;word-wrap:break-word;">' +
    currentComment.get('comment').replace(/<a.*?a>/gi, '') +
    '</div>' +
    '<p>您可以点击 <a style="text-decoration:none; color:#12addb" href="' +
    config.SITE_URL +
    currentComment.get('url') +
    '#comments" target="_blank">查看回复的完整內容 </a>，欢迎再次光临 <a style="text-decoration:none; color:#12addb" href="' +
    config.SITE_URL +
    '" target="_blank">' +
    config.SITE_NAME +
    '</a>。<br>本邮件为系统自动发送，请勿直接回复。</p>  \n' +
    '    </div>  \n' +
    '</div>'
